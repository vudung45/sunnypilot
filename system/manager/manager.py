...
from openpilot.system.manager.helpers import (
    save_bootlog,
    unblock_stdout,
    write_onroad_params,
)
...
def manager_init() -> None:
    ...
    if os.getenv("HANDSMONITORING") is not None:
        params.put_bool(
            "HandsOnWheelMonitoring", bool(int(os.getenv("HANDSMONITORING", "0")))
        )
    ...
    os.environ["GIT_ORIGIN"] = (
        build_metadata.openpilot.git_normalized_origin
    )  # Needed for swaglog
    ...
    while True:
        ...
        for param in ("DoUninstall", "DoShutdown", "DoReboot"):
            if params.get_bool(param):
                shutdown = True
                params.put(
                    "LastManagerExitReason", f"{param} {datetime.datetime.now()}"
                )
